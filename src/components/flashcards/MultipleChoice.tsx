...
export default function MultipleChoice() {
  ...
  // Stable options: memoize with a seed so they don't re-shuffle on every render
  const options = useMemo(() => {
    if (!currentCard) return [];
    const distractors = cards
      .filter(c => c.id !== currentCard.id)
      .slice(0, 10) // take a pool
      .sort(() => currentCard.id.charCodeAt(0) % 2 === 0 ? -1 : 1) // deterministic-ish
      .slice(0, 3)
      .map(c => c.english);
    const all = [...new Set([currentCard.english, ...distractors])];
    // Fisher-Yates shuffle with deterministic seed from card id
  ...
  if (progress.total < 4) {
    return <div className="py-12 text-center text-text-secondary">Need at least 4 saved words for Multiple Choice mode.</div>;
  }
    ...
    <div className="mx-auto max-w-sm space-y-6">
      ...
      {currentCard && (
        <div className="rounded-2xl border border-white/10 bg-bg-card p-8 text-center">
          <p className="font-chinese text-4xl font-medium text-text-primary">{currentCard.chinese}</p>
          <p className="mt-3 text-lg text-text-pinyin">{currentCard.pinyin}</p>
        </div>
      )}

      <div className="grid gap-3">
        {options.map((option, i) => {
          let cls = 'w-full rounded-xl border border-white/10 bg-bg-card px-4 py-3 text-left text-sm font-medium transition-all hover:border-china-red/30';
          if (selected) {
            if (option === currentCard?.english) cls += ' border-green-500 bg-green-500/10 text-green-300';
